...
~~若这种晶格畸变以周期性方式出现，则 $e_g$ 轨道能级的高低分布也将呈周期性。由于电子总是倾向于占据能量较低的轨道，体系中电子的轨道分布将呈现出 **有序排列**，即所谓的 **轨道有序（orbital order）**。~~（这段话可能不够严谨）
...
其中对于 $\sigma\sigma'$ 的求和项，可以分为 $\sigma'=\sigma$ 和 $\sigma'=-\sigma$ 两类进行计算：
